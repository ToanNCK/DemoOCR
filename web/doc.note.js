var OCR = {
    ...
    onEvent: function () {
        ...
        jQuery(document).on("click", ".execOcrForm", function () {
            try {
                ...
                if (typeof data.doctype != 'undefined' && parseInt(data.doctype) > 0 && data.doctype != jQuery(".changeDoctype").val())
                {
                    ...
                    dataDoctype.IDLanguage = jQuery("#IDLanguageOfDoc").val();
        ...
        jQuery(document).on("click", ".execOcrPdf2OtherExtension", function () {
            try {
                var data = jQuery(this).getData();
                jQuery.ajax({
                    type: "POST",
                    ...
                    success: function (response) {
                        if (parseInt(response.status) === 1) {
                            try
                            {
                                var urlWord = Utils.getDomain() + response.text;
                                jQuery(
                                    "<div title='Tải về' id=" + (new Date()).getTime() + ">" +
                                        "<div class='text-center padt20 col-md-12'>" +
                                            "Click <a class='red closeDialog' href='" + urlWord + "'>vào đây</a> để tải file về máy" +
                                        "</div>" +
                                    "</div>"
                                )
                                .dialog({
                    ...
                    success: function (response) {
                        try {
                            if (parseInt(response.reload) == 1) {
                                ...
                                Utils.setError(response.messages);
                            }
    ...
    ocrText: function (field)
    {
        ...
        data.FieldType = 0;
        data.ID = jQuery("#IDViewer").val();
        data.IDLanguage = jQuery("#IDLanguageOfDoc").val();
